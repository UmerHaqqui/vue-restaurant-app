<template>
<img src="../assets/resta.png" alt="img">
<Header />
<div class="form">
    
    <div v-if="loading">
        <Loader txt="aaaaaa" />
    </div>
    <div v-else>

    <div class="signUp">
        <h1>Update Restaurant</h1>
        <!-- <pre>
            {{restaurant.name}}
            {{this.isDisabled}}
        </pre> -->
        <div class="feild">
            <label for="">Enter Your Restaurant's Name</label><br />
            <input @keyup="changeHanlder" type="text" name="name" v-model="restaurant.name" />
        </div>
        <div class="feild">
            <label for="">Enter Your Restaurant's Address</label><br />
            <input type="text" name="address" v-model="restaurant.address" />
        </div>
        <div class="feild">
            <label for="">Enter Your Restaurant's Contact </label><br />
            <input type="number" name="contact" v-model="restaurant.contact" />
        </div>
        <button type="button" v-on:click="updateRestaurant">Save</button>
    </div>
    </div>
 
</div>
</template>

<script>
import Header from './Header.vue'
import axios from 'axios'
import Loader from '@/components/util/Loader.vue'

export default {
    name: 'Update',
    components: {
        Header,
        Loader
    },
    data() {
        return {
            restaurant: {
                name: '',
                address: '',
                contact: ''
            },
            isDisabled: true,
            loading: false
        }
    },
    methods: {
        changeHanlder() {
            if(this.restaurant.name.length !== 0) {
                this.isDisabled = false
            } else {
                this.isDisabled = true
            }
            console.log("xdd", this.isDisabled)
            
            console.log("NN", this.restaurant.name.length)
        },
        async updateRestaurant() {
            this.loading = true
            const result = await axios.put("http://localhost:3000/restaurant/" + this.$route.params.id, {
                name: this.restaurant.name,
                address: this.restaurant.address,
                contact: this.restaurant.contact
            });
            if (result.status == 200) {

                setTimeout(()=>{
                    this.loading = false

                        this.$router.push({
                            name: 'Home'
                        })

                }, 2000)

    
            }
        }
    },
    async mounted() {
        let user = localStorage.getItem('user-info');
        if (!user) {
            this.$router.push({
                name: 'SignUp'
            })
        }
        const result = await axios.get("http://localhost:3000/restaurant/" + this.$route.params.id);
        this.restaurant = result.data
    }
}
</script>

<style scoped>
[disabled] {
    opacity: .2;
}
img {
    width: 100px;
}

.form {
    width: 50%;
    margin: -5% 0 0 25%;
}

.form .signUp {
    width: 100%;
    background: rgb(231, 230, 230);
    border-radius: 10px;
}

.form .signUp h1 {
    color: rgb(73, 73, 73);
    margin-top: 100px;
    padding: 50px 0 10px 10%;
}

.form .signUp .feild {
    margin-left: 0px;
    margin-top: 20px;
}

.form .signUp .feild label {
    font-size: 18px;
    font-weight: 500;
    margin: 0 0 10px 10%;
    color: rgb(73, 73, 73);
}

.form .signUp .feild input {
    width: 80%;
    height: 35px;
    border: none;
    border-radius: 8px;
    margin-left: 10%;
}

.form .signUp button {
    margin: 30px 0 20px 10%;
    padding: 10px 30px;
    font-size: 18px;
    color: #fff;
    background: rgb(73, 73, 73);
    border: none;
    border-radius: 8px;
    font-weight: 700;
    cursor: pointer;
}

.form .signUp button:hover {
    color: #fff;
    background: rgb(190, 189, 189);
}
</style>
